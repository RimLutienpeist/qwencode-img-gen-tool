...
- `[1]`: 数字 - 帧数（如 4, 8, 16）
- `[2]`: 字符串 - 图像主体描述（如 "骑士角色"）
- `[3]`: 字符串 - 动画内容描述（如 "行走动画从站立到跑步"）

**⚠️ 重要：** 使用序列帧数组格式时，**不再需要** `description` 字段！工具会自动组合生成提示词。
...
```json
{
  ...
  "name": "knight_walk",
  "size": "2048x512",
  "is_sheet": [true, 4, "骑士角色", "行走动画从站立到跑步"]
}
```

生成的提示词将是：`骑士角色，序列帧设计，连续帧动画，动画分解，帧与帧之间动画连贯，帧均匀分布，共 4 帧，动画内容: 行走动画从站立到跑步`
...
[
  {
    ...
    "style": "cartoon",
    "size": "2048x2048",
    "is_sheet": [true, 8, "火焰爆炸特效", "从小火花到大爆炸"]
  },
  {
    "name": "hero_run",
    "category": "sheet_char",
...
**适用场景：**
- `sheet_char` + 数组格式 → 角色动画序列帧（行走、攻击、跳跃等）
- `effect` + 数组格式 → 特效动画序列帧（爆炸、火焰、闪电等）

### size（图像尺寸）

**智能尺寸处理机制：**

工具会根据目标尺寸**和使用的模型**自动选择最佳生成策略。不同模型支持的尺寸范围不同：

**模型尺寸支持范围：**

| 模型 | 最小尺寸 | 最大尺寸 | 说明 |
|------|---------|---------|------|
| `doubao-seedream-3-0-t2i-250415` | 512x512 | 2048x2048 | SeedreamV3 标准模型（默认） |
| `doubao-seedream-4-0-250828` | 1280x720 | 4096x4096 | SeedreamV4 高清模型 |
| `doubao-seedream-4-5-251128` | 2560x1440 | 4096x4096 | SeedreamV4.5 超高清模型 |
...
1. **在模型支持范围内**
   - API 直接使用目标尺寸生成
   - 无需后处理，**最佳质量**
   - 例如（V3 模型）：`1024x1024`、`1920x1080`、`2048x2048`

2. **小于模型最小尺寸**
   - 自动放大到最小尺寸范围内（2 的幂次倍数）
   - 生成后缩小回目标尺寸
   - 例如（V3 模型）：`256x256` → 生成 `512x512` → 缩小为 `256x256`

3. **大于模型最大尺寸**
   - 自动缩小到最大尺寸范围内（2 的幂次倍数）
   - 生成后放大到目标尺寸
   - 例如（V3 模型）：`4096x4096` → 生成 `2048x2048` → 放大为 `4096x4096`
...
**说明：** 选择用于图像生成的 AI 模型，不同模型有不同的尺寸支持范围和生成特点

**可用模型：**

| 模型名称 | 版本 | 最小尺寸 | 最大尺寸 | 特点 |
|---------|------|---------|---------|------|
| `doubao-seedream-3-0-t2i-250415` | V3 | 512x512 | 2048x2048 | 标准模型（默认），适合常规游戏素材 |
| `doubao-seedream-4-0-250828` | V4 | 1280x720 | 4096x4096 | 高清模型，支持更大尺寸，质量更好 |
| `doubao-seedream-4-5-251128` | V4.5 | 2560x1440 | 4096x4096 | 超高清模型，最高质量，适合大型背景和高清素材 |

**配置方式：**

1. **全局配置**（推荐）- 在 `main.py` 中修改：
   ```python
   MODEL_NAME = "doubao-seedream-3-0-t2i-250415"  # 修改这里
   ```

2. **单个任务指定** - 在 `tasks.json` 中为特定任务指定模型：
   ```json
   {
     "description": "超高清游戏背景",
     "category": "background",
     "size": "4096x2160",
     "model": "doubao-seedream-4-5-251128"  // 单独指定模型
   }
   ```

**使用建议：**

- **V3 模型**：适合大多数游戏素材（角色、UI、道具等）
- **V4 模型**：需要更高分辨率或更好质量时使用
- **V4.5 模型**：用于大型背景图、CG 插画等高清场景

**注意事项：**
- 高版本模型生成速度可能较慢
- 确保目标尺寸在模型支持范围内以获得最佳效果
- 工具会自动根据模型调整 API 调用尺寸
